...
import tp.p1.control.Exceptions.InvalidPosition;
...
public class Game {
	...
	private Random r;
	private Level levelio;
	private Board board;
	private String seed, level, help;
	private static final String InvalidPositionTxt = ") is an invalid position.";
	...
	public boolean addPlantToGame(Plant plant, String xstr, String ystr) throws InvalidPosition, NotEnoughCoins {
		boolean planted = false;
		
		int x = Integer.parseInt(xstr), y = Integer.parseInt(ystr);
		if (suncoinmanag.compare(board.getCost(plant))) {
			if (x >= 0 && x < 4 && y >= 0 && y < 7 && emptyTile(x, y)) {
				...
				throw new InvalidPosition("(" + x + ", " + y + InvalidPositionTxt);
		}
		return planted;
	}
	...
	public void load(String[] line) throws FileContentsException {
		...
		if (Integer.parseInt(line[1]) < 0)
			...
			suncoinmanag.equalSuncoins(Integer.parseInt(line[1]));
		if (Level.parse(line[2]) == null)
			throw new FileContentsException();
		else
			level = Level.parse(line[2]).name();
        ...
        for(int i = 4; i < line.length && line[i] != null; i++) {
        	...
        	else if (ZombieFactory.getZombie(line[i].charAt(0) + "") != null) {
	...
	void equalSuncoins(int num) {
		suncoinmanag.equalSuncoins(num);
	}
	
	public void setZombiesLeftToAppear(int zombies) {
		zombmanag.setZombiesLeftToAppear(zombies);
	}
	
	public void setCycle(int cycleCount) {
		this.cycleCount = cycleCount;
	}

	public Game MemoryBackUp() {
		Game backup;
		if (seed == null)
			backup = new Game(levelio, printer);
		else
			backup = new Game(levelio, Integer.parseInt(seed), printer);
		backup.setCycle(cycleCount);
		backup.equalSuncoins(suncoinmanag.getSuncoins());
		backup.setZombiesLeftToAppear(zombmanag.getZombiesLeftToAppear());
		backup.board = board.RestoreLists();
		return backup;
	}

	public void restoreState(Game backup) {
		cycleCount = backup.cycleCount;
		board = backup.board;
		suncoinmanag.equalSuncoins(backup.getSunCoins());
		zombmanag = backup.zombmanag;
	}
}
